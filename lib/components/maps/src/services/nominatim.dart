...
class NominatimService {
  Future<List<Map>> getAddressLatLng(String address) async {
    var url =
        "https://nominatim.openstreetmap.org/search?q=${address.replaceAll(RegExp(' '), '+')}&format=json&addressdetails=1&country=Netherlands";
    print(url);
  ...
  Future<List<Map>> getAddressOnlyNetherlands(String address) async {
    var url =
        "https://nominatim.openstreetmap.org/search?street=${address.replaceAll(RegExp(' '), '+')}&format=json&addressdetails=1&country=Netherlands";
    print(url);
    var response = await http.get(url);
    print('Response status: ${response.statusCode}');
    print('Response body: ${response.body}');
